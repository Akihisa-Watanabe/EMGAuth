...
def normal_test_split(
    ...
    # =======================テスト用の負例データを抽出=======================
    df_attacker = df.query("user=={}".format(attacker))
    g = df_attacker.groupby("gesture", group_keys=True)
    L_test_each_gesture = math.ceil(L_test_verify_user / len(C_gestures))
    df_test_attacker = g.apply(
